    ...
    if request.method == 'POST':
        ...
        if student_form.is_valid():
            ...
            for course_id, total_mark_id in zip(course, total_mark):
                ...
                # print('Course ID: ', course_id, ' | |  Mark ID: ', total_mark_id)
            ...
            models.Semester_Result.objects.create(
                student=student,
                semester=models.Semester.objects.get(id=semester),
                session=models.Session.objects.get(id=session),
                gpa=gpa,
...
def semester_result_list(request):
    ...
    if query:
        ...
        students = models.Semester_Result.objects.filter(level__icontains=query, department__icontains=query2, semester__name__icontains=query3, session__name__icontains=query4)
...
def grade_export_students_excel(request):
    ...
    sheet.append(['Student Name', 'Matric Number', 'Session', 'Semester', 'Level', 'Department', 'GPA', 'Remark'])

    # Write the data rows
    for student in students:
        sheet.append([student.student.name, student.student.registration_number, student.session.name, student.semester.name, student.level,
